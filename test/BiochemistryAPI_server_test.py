...
class BiochemistryAPITest(unittest.TestCase):
    ...
    def test_bad_input(self):
        ...
        with self.assertRaisesRegexp(ValueError, "Invalid fingerprint type"):
            self.getImpl().similarity_search(
                self.ctx, {"query": "C(=O)O", "fp_type": "foo"}
            )
        with self.assertRaisesRegexp(ValueError, "Invalid min_similarity"):
            self.getImpl().similarity_search(
                self.ctx, {"query": "C", "min_similarity": 1.1}
            )
        with self.assertRaisesRegexp(ValueError, "Invalid min_similarity"):
            self.getImpl().similarity_search(
                self.ctx, {"query": "C", "min_similarity": "0.8"}
            )

    def test_get_compounds(self):
        cpds = self.getImpl().get_compounds(
            self.ctx,
            {"compounds": ["48/1/1/compounds/id/cpd00011", "cpd00002", "cpd00007"]},
        )[0]
        assert len(cpds) == 3
    ...
    def test_get_reactions(self):
        rxns = self.getImpl().get_reactions(
            self.ctx,
            {"reactions": ["48/1/1/reactions/id/rxn00011", "rxn00002", "rxn00007"]},
        )[0]
        assert len(rxns) == 3
    ...
    def test_search_compounds(self):
        ...
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0]["id"], "cpd00007")
        res = self.getImpl().search_compounds(
            self.ctx, {"query": "Pyruvate", "limit": 5}
        )[0]
    ...
    def test_depict_compounds(self):
        svgs = self.getImpl().depict_compounds(
            self.ctx,
            {"structures": ["C(=O)O", "InChI=1S/C2H4O2/c1-2(3)4/h1H3,(H,3,4)", "foo"]},
        )[0]
    ...
    def test_calculate_3D_pdb(self):
        mols = self.getImpl().calculate_3D_coords(
            self.ctx, {"structures": ["C(=O)O"], "optimize": "True", "output": "pdb"}
        )[0]
        self.assertEqual(len(mols), 1)
        print(mols[0])
        self.assertIn(
            "HETATM    2  O1  UNL     1      -1.109  -0.233   0.000  1.00  0.00",
            mols[0],
        )
