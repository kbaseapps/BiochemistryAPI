...
compound_headers = ["id","abbreviation","name","formula","mass","source","structure","charge","is_core",
                    "is_obsolete","is_cofactor","deltaG","deltaGErr","pka","pkb","aliases"]

cpd_header_translation={"deltaG":"deltag",
                        "deltaGErr":"deltagerr",
                        "structure":"inchikey"}
...
reaction_headers = ["id","abbreviation","name","code","stoichiometry","is_transport","equation","definition",
                    "reversibility","direction","abstract_reaction","pathways","aliases","ec_numbers",
...
with open('Compounds_Aliases.tsv','w') as cpda_fh:
    ...
    file = urlopen(MSD_git_url+MSD_commit+"/Biochemistry/Aliases/Unique_ModelSEED_Compound_Aliases.txt")
    header=1
...
with open('Reactions_Aliases.tsv','w') as rxna_fh:
    ...
    file = urlopen(MSD_git_url+MSD_commit+"/Biochemistry/Aliases/Unique_ModelSEED_Reaction_Aliases.txt")
    header=1
    for line in file.readlines():
        if(header==1):
            header-=1
...
with open('Enzyme_Class_Reactions_Aliases.tsv','w') as rxnec_fh:
    ...
    file = urlopen(MSD_git_url+MSD_commit+"/Biochemistry/Aliases/Unique_ModelSEED_Reaction_ECs.txt")
    header=1
    for line in file.readlines():
        if(header==1):
            header-=1
